...
use _PhpScoperf9ff1c3447b1\Nette\Utils\DateTime;
...
final class AddTagToChangelogReleaseWorker implements \Symplify\MonorepoBuilder\Release\Contract\ReleaseWorker\ReleaseWorkerInterface
{
    ...
    public function work(\_PhpScoperf9ff1c3447b1\PharIo\Version\Version $version) : void
    {
        ...
        $changelogFileContent = \_PhpScoperf9ff1c3447b1\Nette\Utils\Strings::replace($changelogFileContent, self::UNRELEASED_HEADLINE_REGEX, '## ' . $newHeadline);
        $this->smartFileSystem->dumpFile($changelogFilePath, $changelogFileContent);
    }
    public function getDescription(\_PhpScoperf9ff1c3447b1\PharIo\Version\Version $version) : string
    {
        $newHeadline = $this->createNewHeadline($version);
        return \sprintf('Change "Unreleased" in `CHANGELOG.md` to "%s"', $newHeadline);
    }
    private function createNewHeadline(\_PhpScoperf9ff1c3447b1\PharIo\Version\Version $version) : string
    {
        $dateTime = new \_PhpScoperf9ff1c3447b1\Nette\Utils\DateTime();

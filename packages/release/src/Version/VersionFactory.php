...
final class VersionFactory
{
    ...
    public function createValidVersion(string $versionArgument, string $stage) : \_PhpScoperef96225926e2\PharIo\Version\Version
    {
        ...
        $version = new \_PhpScoperef96225926e2\PharIo\Version\Version($versionArgument);
        $this->releaseGuard->guardVersion($version, $stage);
        return $version;
    }
    private function resolveNextVersionByVersionKind(string $versionKind) : \_PhpScoperef96225926e2\PharIo\Version\Version
    {
        // get current version
        $mostRecentVersion = $this->mostRecentTagResolver->resolve(\getcwd());
        if ($mostRecentVersion === null) {
            // the very first tag
            return new \_PhpScoperef96225926e2\PharIo\Version\Version('v0.1.0');
        }
        $mostRecentVersion = new \_PhpScoperef96225926e2\PharIo\Version\Version($mostRecentVersion);
        ...
        return new \_PhpScoperef96225926e2\PharIo\Version\Version(\sprintf('v%d.%d.%d', $value, $currentMinorVersion, $currentPatchVersion));
    }
}

...
use _PhpScoperf4d251e01a80\Jean85\PrettyVersions;
use _PhpScoperf4d251e01a80\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoperf4d251e01a80\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoperf4d251e01a80\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoperf4d251e01a80\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoperf4d251e01a80\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoperf4d251e01a80\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoperf4d251e01a80\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoperf4d251e01a80\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

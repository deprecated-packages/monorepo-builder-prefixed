...
use _PhpScoper6da0ff96db02\Jean85\PrettyVersions;
use _PhpScoper6da0ff96db02\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper6da0ff96db02\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper6da0ff96db02\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper6da0ff96db02\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper6da0ff96db02\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper6da0ff96db02\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper6da0ff96db02\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper6da0ff96db02\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

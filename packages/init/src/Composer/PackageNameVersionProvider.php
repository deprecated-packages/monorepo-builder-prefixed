...
use _PhpScoper3a64dbb4e14f\Jean85\PrettyVersions;
use _PhpScoper3a64dbb4e14f\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper3a64dbb4e14f\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper3a64dbb4e14f\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper3a64dbb4e14f\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper3a64dbb4e14f\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper3a64dbb4e14f\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper3a64dbb4e14f\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper3a64dbb4e14f\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

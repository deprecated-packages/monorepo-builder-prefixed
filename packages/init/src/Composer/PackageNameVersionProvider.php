...
use _PhpScoper2f9b0ac741dd\Jean85\PrettyVersions;
use _PhpScoper2f9b0ac741dd\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper2f9b0ac741dd\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper2f9b0ac741dd\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper2f9b0ac741dd\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper2f9b0ac741dd\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper2f9b0ac741dd\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper2f9b0ac741dd\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper2f9b0ac741dd\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

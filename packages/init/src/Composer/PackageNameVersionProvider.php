...
use _PhpScoper0217ecc5969a\Jean85\PrettyVersions;
use _PhpScoper0217ecc5969a\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper0217ecc5969a\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper0217ecc5969a\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper0217ecc5969a\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper0217ecc5969a\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper0217ecc5969a\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper0217ecc5969a\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper0217ecc5969a\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

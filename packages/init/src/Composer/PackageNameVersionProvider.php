...
use _PhpScoper7606a7c2799b\Jean85\PrettyVersions;
use _PhpScoper7606a7c2799b\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper7606a7c2799b\PharIo\Version\InvalidVersionException;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper7606a7c2799b\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper7606a7c2799b\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper7606a7c2799b\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper7606a7c2799b\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper7606a7c2799b\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper7606a7c2799b\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

...
use _PhpScoper7b254e6a7108\Jean85\PrettyVersions;
use _PhpScoper7b254e6a7108\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoper7b254e6a7108\PharIo\Version\InvalidVersionException;
use _PhpScoper7b254e6a7108\PharIo\Version\Version;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoper7b254e6a7108\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoper7b254e6a7108\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoper7b254e6a7108\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoper7b254e6a7108\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoper7b254e6a7108\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoper7b254e6a7108\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

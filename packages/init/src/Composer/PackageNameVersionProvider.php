...
use _PhpScoperd410cf9baa15\Jean85\PrettyVersions;
use _PhpScoperd410cf9baa15\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoperd410cf9baa15\PharIo\Version\InvalidVersionException;
use _PhpScoperd410cf9baa15\PharIo\Version\Version;
...
final class PackageNameVersionProvider
{
    ...
    public function provide(string $packageName) : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoperd410cf9baa15\Jean85\PrettyVersions::getVersion($packageName)->getPrettyVersion();
            $version = new \_PhpScoperd410cf9baa15\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoperd410cf9baa15\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractFromComposer(string $packageName) : ?\_PhpScoperd410cf9baa15\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoperd410cf9baa15\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === $packageName) {
                    return new \_PhpScoperd410cf9baa15\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

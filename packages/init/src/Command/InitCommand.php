...
use _PhpScoperda849baa4a45\Jean85\PrettyVersions;
use _PhpScoperda849baa4a45\Nette\Utils\Json as NetteJson;
use OutOfBoundsException;
use _PhpScoperda849baa4a45\PharIo\Version\InvalidVersionException;
...
final class InitCommand extends \_PhpScoperda849baa4a45\Symfony\Component\Console\Command\Command
{
    /**
     * @var string
     */
    ...
    public function __construct(\_PhpScoperda849baa4a45\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, \Symplify\SmartFileSystem\SmartFileSystem $smartFileSystem)
    {
        parent::__construct();
    ...
    protected function configure() : void
    {
        ...
        $this->addArgument(self::OUTPUT, \_PhpScoperda849baa4a45\Symfony\Component\Console\Input\InputArgument::OPTIONAL, 'Directory to generate monorepo into.', \getcwd());
    }
    ...
    private function getMonorepoBuilderVersion() : string
    {
        $version = null;
        try {
            $prettyVersion = \_PhpScoperda849baa4a45\Jean85\PrettyVersions::getVersion('symplify/monorepo-builder')->getPrettyVersion();
            $version = new \_PhpScoperda849baa4a45\PharIo\Version\Version(\str_replace('x-dev', '0', $prettyVersion));
        } catch (\OutOfBoundsException|\_PhpScoperda849baa4a45\PharIo\Version\InvalidVersionException $exceptoin) {
     ...
     */
    private function extractMonorepoBuilderVersionFromComposer() : ?\_PhpScoperda849baa4a45\PharIo\Version\Version
    {
        ...
        if (\is_file($installedJsonFilename)) {
            ...
            $installedJson = \_PhpScoperda849baa4a45\Nette\Utils\Json::decode($installedJsonFileContent);
            foreach ($installedJson as $installedPackage) {
                if ($installedPackage->name === 'symplify/monorepo-builder') {
                    return new \_PhpScoperda849baa4a45\PharIo\Version\Version($installedPackage->version);
                }
            }
        }
        return null;

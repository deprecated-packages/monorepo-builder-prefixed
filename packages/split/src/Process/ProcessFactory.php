...
final class ProcessFactory
{
    ...
    public function createSubsplit(?string $theMostRecentTag, string $directory, string $remoteRepository, string $branch) : \_PhpScoperedfa898a454f\Symfony\Component\Process\Process
    {
     ...
     */
    private function createProcessFromCommandLine(array $commandLine, string $directory) : \_PhpScoperedfa898a454f\Symfony\Component\Process\Process
    {
        $directory = $this->subsplitCacheDirectory . \DIRECTORY_SEPARATOR . \_PhpScoperedfa898a454f\Nette\Utils\Strings::webalize($directory);
        $this->smartFileSystem->remove($directory);
        $this->smartFileSystem->mkdir($directory);
        return new \_PhpScoperedfa898a454f\Symfony\Component\Process\Process($commandLine, $directory, null, null, null);
    }
}

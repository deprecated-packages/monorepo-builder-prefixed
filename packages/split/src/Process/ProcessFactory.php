...
final class ProcessFactory
{
    ...
    public function createSubsplit(?string $theMostRecentTag, string $directory, string $remoteRepository, string $branch) : \_PhpScopera89333f3e9e2\Symfony\Component\Process\Process
    {
     ...
     */
    private function createProcessFromCommandLine(array $commandLine, string $directory) : \_PhpScopera89333f3e9e2\Symfony\Component\Process\Process
    {
        $directory = $this->subsplitCacheDirectory . \DIRECTORY_SEPARATOR . \_PhpScopera89333f3e9e2\Nette\Utils\Strings::webalize($directory);
        $this->smartFileSystem->remove($directory);
        $this->smartFileSystem->mkdir($directory);
        return new \_PhpScopera89333f3e9e2\Symfony\Component\Process\Process($commandLine, $directory, null, null, null);
    }
}

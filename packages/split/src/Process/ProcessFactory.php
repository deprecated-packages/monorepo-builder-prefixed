...
final class ProcessFactory
{
    ...
    public function createSubsplit(?string $theMostRecentTag, string $directory, string $remoteRepository, string $branch) : \_PhpScoper650fbdd90fa0\Symfony\Component\Process\Process
    {
     ...
     */
    private function createProcessFromCommandLine(array $commandLine, string $directory) : \_PhpScoper650fbdd90fa0\Symfony\Component\Process\Process
    {
        $directory = $this->subsplitCacheDirectory . \DIRECTORY_SEPARATOR . \_PhpScoper650fbdd90fa0\Nette\Utils\Strings::webalize($directory);
        $this->smartFileSystem->remove($directory);
        $this->smartFileSystem->mkdir($directory);
        return new \_PhpScoper650fbdd90fa0\Symfony\Component\Process\Process($commandLine, $directory, null, null, null);
    }
}

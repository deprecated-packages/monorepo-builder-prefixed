...
final class DirectoryToRepositoryProvider
{
    ...
    public function provide() : array
    {
        ...
        foreach ($directoriesToRepositories as $directory => $repository) {
            if (!\_PhpScoper7371f586a1d2\Nette\Utils\Strings::contains($directory, '*')) {
    ...
    private function resolveAsteriskDirectory(string $directory, string $repository) : array
    {
        ...
        foreach ($foundDirectories as $foundDirectory) {
            ...
            $exclusiveName = (string) \_PhpScoper7371f586a1d2\Nette\Utils\Strings::after((string) $foundDirectory, $patternWithoutAsterisk);
            $exclusiveName = $this->convertRepositoryToDesiredFormat($exclusiveName);
            $targetRepository = \_PhpScoper7371f586a1d2\Nette\Utils\Strings::replace($repository, self::ASTERISK_REGEX, $exclusiveName);

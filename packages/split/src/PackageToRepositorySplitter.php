...
use _PhpScopere809b8269e19\Symfony\Component\Process\Process;
...
final class PackageToRepositorySplitter
{
     ...
     */
    private $gitManager;
    public function __construct(\_PhpScopere809b8269e19\Symfony\Component\Console\Style\SymfonyStyle $symfonyStyle, \Symplify\SmartFileSystem\FileSystemGuard $fileSystemGuard, \Symplify\MonorepoBuilder\Split\Process\ProcessFactory $processFactory, \Symplify\MonorepoBuilder\Split\Git\GitManager $gitManager)
    {
     ...
     */
    public function splitDirectoriesToRepositories(array $splitConfig, string $rootDirectory, ?string $branch = null, ?int $maxProcesses = null, ?string $tag = null) : void
    {
        if ($tag === null) {
        ...
        foreach ($splitConfig as $localDirectory => $remoteRepository) {
            ...
            $remoteRepositoryWithGithubKey = $this->gitManager->completeRemoteRepositoryWithGithubToken($remoteRepository);
            $process = $this->processFactory->createSubsplit($tag, $localDirectory, $remoteRepositoryWithGithubKey, $branch);
            $this->symfonyStyle->note('Running: ' . $process->getCommandLine());
            $process->start();
            $this->activeProcesses[] = $process;
            $this->splitProcessInfos[] = new \Symplify\MonorepoBuilder\Split\ValueObject\SplitProcessInfo($process, $localDirectory, $remoteRepository);
    ...
    private function reportFinishedProcesses() : void
    {
        foreach ($this->splitProcessInfos as $processInfo) {
            ...
            if (!$process->isSuccessful()) {
                ...
                throw new \Symplify\MonorepoBuilder\Split\Exception\PackageToRepositorySplitException($message);
            }
            $successMessage = \sprintf('Push of "%s" directory to "%s" repository was successful: %s "%s"', $processInfo->getLocalDirectory(), $processInfo->getRemoteRepository(), \PHP_EOL . \PHP_EOL, $process->getOutput());

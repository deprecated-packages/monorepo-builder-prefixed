...
final class GitManager
{
    ...
    public function __construct(\Symplify\MonorepoBuilder\Release\Process\ProcessRunner $processRunner, \Symplify\PackageBuilder\Parameter\ParameterProvider $parameterProvider)
    {
        $this->processRunner = $processRunner;
        $this->githubToken = (string) $parameterProvider->provideStringParameter(\Symplify\MonorepoBuilder\ValueObject\Option::GITHUB_TOKEN);
    }
    public function doesBranchExistOnRemote(string $branch) : bool
    {
        $command = ['git', 'branch', '-a'];
        $result = $this->processRunner->run($command);
        return \_PhpScoper9fc59dd81894\Nette\Utils\Strings::contains($result, \sprintf('remotes/origin/%s', $branch));
    }
    public function getCurrentBranch() : string
    {
        $command = ['git', 'rev-parse', '--abbrev-ref', 'HEAD'];
        $currentBranch = \trim($this->processRunner->run($command));
    ...
    {
        $command = ['git', 'push', '--set-upstream', 'origin', \trim($branch)];
    ...
    public function doTagsHaveCommitterDate() : bool
    {
        ...
        return (bool) \_PhpScoper9fc59dd81894\Nette\Utils\Strings::match($result, self::COMMITER_DATE_START_REGEX);
    }
    /**
    ...
    public function completeRemoteRepositoryWithGithubToken(string $remoteRepository) : string
    {
        ...
        $partAfterAt = \_PhpScoper9fc59dd81894\Nette\Utils\Strings::replace($partAfterAt, self::SEMICOLON_REGEX, '/');

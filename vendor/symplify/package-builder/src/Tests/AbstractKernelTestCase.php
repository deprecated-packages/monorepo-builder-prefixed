 ...
 */
abstract class AbstractKernelTestCase extends \_PhpScoperd0ff379dfdc7\PHPUnit\Framework\TestCase
{
    /**
     * @var KernelInterface
     */
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \_PhpScoperd0ff379dfdc7\Symfony\Component\HttpKernel\KernelInterface
    {
        $configsHash = $this->resolveConfigsHash($configs);
        $this->ensureKernelShutdown();
        $kernel = new $kernelClass('test_' . $configsHash, \true);
        if (!$kernel instanceof \_PhpScoperd0ff379dfdc7\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \_PhpScoperd0ff379dfdc7\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    private function ensureIsConfigAwareKernel(\_PhpScoperd0ff379dfdc7\Symfony\Component\HttpKernel\KernelInterface $kernel) : void
    {
    ...
    private function bootAndReturnKernel(\_PhpScoperd0ff379dfdc7\Symfony\Component\HttpKernel\KernelInterface $kernel) : \_PhpScoperd0ff379dfdc7\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \_PhpScoperd0ff379dfdc7\Psr\Container\ContainerInterface) {
            throw new \Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\_PhpScoperd0ff379dfdc7\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\_PhpScoperd0ff379dfdc7\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\_PhpScoperd0ff379dfdc7\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }

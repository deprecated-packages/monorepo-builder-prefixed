 ...
 */
abstract class AbstractKernelTestCase extends \_PhpScopercdfd316eda69\PHPUnit\Framework\TestCase
{
    /**
     * @var KernelInterface
     */
     ...
     */
    protected function bootKernelWithConfigs(string $kernelClass, array $configs) : \_PhpScopercdfd316eda69\Symfony\Component\HttpKernel\KernelInterface
    {
        $configsHash = $this->resolveConfigsHash($configs);
        $this->ensureKernelShutdown();
        $kernel = new $kernelClass('test_' . $configsHash, \true);
        if (!$kernel instanceof \_PhpScopercdfd316eda69\Symfony\Component\HttpKernel\KernelInterface) {
    ...
    protected function ensureKernelShutdown() : void
    {
        if (static::$kernel !== null) {
            ...
            if ($kernel !== null) {
                ...
                if ($container instanceof \_PhpScopercdfd316eda69\Symfony\Contracts\Service\ResetInterface) {
                    $container->reset();
                }
            }
        }
    ...
    private function ensureIsConfigAwareKernel(\_PhpScopercdfd316eda69\Symfony\Component\HttpKernel\KernelInterface $kernel) : void
    {
    ...
    private function bootAndReturnKernel(\_PhpScopercdfd316eda69\Symfony\Component\HttpKernel\KernelInterface $kernel) : \_PhpScopercdfd316eda69\Symfony\Component\HttpKernel\KernelInterface
    {
        $kernel->boot();
        ...
        if (!$container instanceof \_PhpScopercdfd316eda69\Psr\Container\ContainerInterface) {
            throw new \Symplify\SymplifyKernel\Exception\ShouldNotHappenException();
        }
        // has output? keep it silent out of tests
        if ($container->has(\_PhpScopercdfd316eda69\Symfony\Component\Console\Style\SymfonyStyle::class)) {
            $symfonyStyle = $container->get(\_PhpScopercdfd316eda69\Symfony\Component\Console\Style\SymfonyStyle::class);
            $symfonyStyle->setVerbosity(\_PhpScopercdfd316eda69\Symfony\Component\Console\Output\OutputInterface::VERBOSITY_QUIET);
        }

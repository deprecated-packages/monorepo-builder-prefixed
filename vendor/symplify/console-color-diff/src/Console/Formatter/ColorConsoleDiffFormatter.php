...
final class ColorConsoleDiffFormatter
{
    ...
    {
        $escapedDiff = \_PhpScoper9b5fd937b4d9\Symfony\Component\Console\Formatter\OutputFormatter::escape(\rtrim($diff));
        $escapedDiffLines = \_PhpScoper9b5fd937b4d9\Nette\Utils\Strings::split($escapedDiff, self::NEWLINES_REGEX);
    ...
    {
        return \_PhpScoper9b5fd937b4d9\Nette\Utils\Strings::replace($string, self::PLUS_START_REGEX, '<fg=green>$1</fg=green>');
    }
    private function makeMinusLinesRed(string $string) : string
    {
        return \_PhpScoper9b5fd937b4d9\Nette\Utils\Strings::replace($string, self::MINUT_START_REGEX, '<fg=red>$1</fg=red>');
    }
    private function makeAtNoteCyan(string $string) : string
    {
        return \_PhpScoper9b5fd937b4d9\Nette\Utils\Strings::replace($string, self::AT_START_REGEX, '<fg=cyan>$1</fg=cyan>');
    }
}

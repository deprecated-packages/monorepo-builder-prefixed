 ...
 */
namespace _PhpScoper8aa7c2bece07\SebastianBergmann\Diff\Utils;
 ...
 * @coversNothing
 */
final class UnifiedDiffAssertTraitIntegrationTest extends \_PhpScoper8aa7c2bece07\PHPUnit\Framework\TestCase
{
    ...
    public function testValidPatches(string $fileFrom, string $fileTo) : void
    {
        ...
        $this->assertSame(
            1,
            // means `diff` found a diff between the files we gave it
            $exitCode,
            \sprintf("Command exec. was not successful:\n\"%s\"\nOutput:\n\"%s\"\nStdErr:\n\"%s\"\nExit code %d.\n", $command, $p->getOutput(), $p->getErrorOutput(), $p->getExitCode())
        );
        $this->assertValidUnifiedDiffFormat(\_PhpScoper8aa7c2bece07\SebastianBergmann\Diff\Utils\FileUtils::getFileContent($this->filePatch));
    }
    /**

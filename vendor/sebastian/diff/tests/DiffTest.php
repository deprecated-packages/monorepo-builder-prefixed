 ...
 */
final class DiffTest extends \_PhpScopera31d0d6ff47a\PHPUnit\Framework\TestCase
{
    public function testGettersAfterConstructionWithDefault() : void
    {
        $from = 'line1a';
        $to = 'line2a';
        $diff = new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Diff($from, $to);
    ...
    public function testGettersAfterConstructionWithChunks() : void
    {
        $from = 'line1b';
        $to = 'line2b';
        $chunks = [new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Chunk(), new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Chunk(2, 3)];
        $diff = new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Diff($from, $to, $chunks);
        $this->assertSame($from, $diff->getFrom());
        $this->assertSame($to, $diff->getTo());
    ...
    {
        $diff = new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Diff('line1c', 'line2c');
        $this->assertSame([], $diff->getChunks(), 'Expect chunks to be default value "array()".');
        $chunks = [new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Chunk(), new \_PhpScopera31d0d6ff47a\SebastianBergmann\Diff\Chunk(2, 3)];

...
namespace _PhpScopere846b54512fc\Nette\Neon;
...
final class Decoder
{
    ...
    public function decode(string $input)
    {
        ...
        if ($this->tokens === \false) {
            throw new \_PhpScopere846b54512fc\Nette\Neon\Exception('Invalid UTF-8 sequence.');
        }
        ...
        for (; $n < $count; $n++) {
            ...
            if ($t === ',') {
                ...
                if ($hasValue) {
                    ...
                    $n++;
                    if ($value instanceof \_PhpScopere846b54512fc\Nette\Neon\Entity && $value->value === \_PhpScopere846b54512fc\Nette\Neon\Neon::CHAIN) {
                        \end($value->attributes)->attributes = $this->parse(\false, []);
                    } else {
                        $value = new \_PhpScopere846b54512fc\Nette\Neon\Entity($value, $this->parse(\false, []));
                    }
                } else {
                    $n++;
                ...
                if ($hasValue) {
                    if ($value instanceof \_PhpScopere846b54512fc\Nette\Neon\Entity) {
                        // Entity chaining
                        if ($value->value !== \_PhpScopere846b54512fc\Nette\Neon\Neon::CHAIN) {
                            $value = new \_PhpScopere846b54512fc\Nette\Neon\Entity(\_PhpScopere846b54512fc\Nette\Neon\Neon::CHAIN, [$value]);
                        }
                        $value->attributes[] = new \_PhpScopere846b54512fc\Nette\Neon\Entity($converted);
                    } else {
                        $this->error();
                    }
                } else {
    ...
    private function error(string $message = "Unexpected '%s'")
    {
        ...
        throw new \_PhpScopere846b54512fc\Nette\Neon\Exception(\str_replace('%s', $token, $message) . " on line {$line}, column {$col}.");
    }
}

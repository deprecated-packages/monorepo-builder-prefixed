...
class Finder implements \IteratorAggregate, \Countable
{
    ...
    {
        if ($this->paths) {
            throw new \_PhpScoperfec5e512f2f8\Nette\InvalidStateException('Directory to search has already been specified.');
        }
    ...
    {
        if (!$this->paths) {
            throw new \_PhpScoperfec5e512f2f8\Nette\InvalidStateException('Call in() or from() to specify directory to search.');
    ...
    public function size(string $operator, int $size = null) : self
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?((?:\\d*\\.)?\\d+)\\s*(K|M|G|)B?\\z#i', $operator, $matches)) {
                throw new \_PhpScoperfec5e512f2f8\Nette\InvalidArgumentException('Invalid size predicate format.');
            }
    ...
    public function date(string $operator, $date = null) : self
    {
        if (\func_num_args() === 1) {
            ...
            if (!\preg_match('#^(?:([=<>!]=?|<>)\\s*)?(.+)\\z#i', $operator, $matches)) {
                throw new \_PhpScoperfec5e512f2f8\Nette\InvalidArgumentException('Invalid date predicate format.');
            }
            [, $operator, $date] = $matches;
            $operator = $operator ?: '=';
        }
        $date = \_PhpScoperfec5e512f2f8\Nette\Utils\DateTime::from($date)->format('U');
    ...
    public static function compare($l, string $operator, $r) : bool
    {
        switch ($operator) {
            ...
            case '<>':
                return $l != $r;
            default:
                throw new \_PhpScoperfec5e512f2f8\Nette\InvalidArgumentException("Unknown operator {$operator}.");
        }
    }

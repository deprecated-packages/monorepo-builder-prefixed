 ...
 * </code>
 */
class Html implements \ArrayAccess, \Countable, \IteratorAggregate, \_PhpScoper120e1a259c7c\Nette\Utils\IHtmlString
{
    ...
    public static function el(string $name = null, $attrs = null)
    {
        ...
        if (isset($parts[1])) {
            foreach (\_PhpScoper120e1a259c7c\Nette\Utils\Strings::matchAll($parts[1] . ' ', '#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\\s))?#i') as $m) {
    ...
    {
        if (!$text instanceof \_PhpScoper120e1a259c7c\Nette\Utils\IHtmlString) {
        ...
        foreach ($attrs as $key => $value) {
            if ($value === null || $value === \false) {
                ...
                if (\strncmp($key, 'data-', 5) === 0) {
                    $value = \_PhpScoper120e1a259c7c\Nette\Utils\Json::encode($value);
                } else {
                    $tmp = null;

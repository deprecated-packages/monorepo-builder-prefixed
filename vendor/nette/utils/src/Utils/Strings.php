...
class Strings
{
    ...
    public static function chr(int $code) : string
    {
        if ($code < 0 || $code >= 0xd800 && $code <= 0xdfff || $code > 0x10ffff) {
            throw new \_PhpScoper68e56c1b5bd9\Nette\InvalidArgumentException('Code point must be in range 0x0 to 0xD7FF or 0xE000 to 0x10FFFF.');
        }
    ...
    public static function replace(string $subject, $pattern, $replacement = null, int $limit = -1) : string
    {
        if (\is_object($replacement) || \is_array($replacement)) {
            if (!\is_callable($replacement, \false, $textual)) {
                throw new \_PhpScoper68e56c1b5bd9\Nette\InvalidStateException("Callback '{$textual}' is not callable.");
            }
    ...
    {
        $res = \_PhpScoper68e56c1b5bd9\Nette\Utils\Callback::invokeSafe($func, $args, function (string $message) use($args) : void {
            // compile-time error, not detectable by preg_last_error
            throw new \_PhpScoper68e56c1b5bd9\Nette\Utils\RegexpException($message . ' in pattern: ' . \implode(' or ', (array) $args[0]));
        });
        if (($code = \preg_last_error()) && ($res === null || !\in_array($func, ['preg_filter', 'preg_replace_callback', 'preg_replace'], \true))) {
            throw new \_PhpScoper68e56c1b5bd9\Nette\Utils\RegexpException((\_PhpScoper68e56c1b5bd9\Nette\Utils\RegexpException::MESSAGES[$code] ?? 'Unknown error') . ' (pattern: ' . \implode(' or ', (array) $args[0]) . ')', $code);
        }
        return $res;
    }
}

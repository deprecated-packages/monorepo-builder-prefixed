...
class Validators
{
    ...
    public static function assert($value, string $expected, string $label = 'variable') : void
    {
        if (!static::is($value, $expected)) {
            ...
            throw new \_PhpScoper4cbad741edc5\Nette\Utils\AssertionException("The {$label} expects to be {$expected}, {$type} given.");
        }
    }
    /**
    ...
    public static function assertField(array $arr, $field, string $expected = null, string $label = "item '%' in array") : void
    {
        if (!\array_key_exists($field, $arr)) {
            throw new \_PhpScoper4cbad741edc5\Nette\Utils\AssertionException('Missing ' . \str_replace('%', $field, $label) . '.');
        } elseif ($expected) {
    ...
    {
        return \_PhpScoper4cbad741edc5\Nette\Utils\Arrays::isList($value);
    }
    /**

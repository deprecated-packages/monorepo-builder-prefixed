...
use _PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\DuplicateKeyException;
 ...
 */
class PrototypedArrayNode extends \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\ArrayNode
{
    protected $prototype;
     ...
     */
    public function setPrototype(\_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\PrototypeNodeInterface $node)
    {
        $this->prototype = $node;
    }
    /**
     ...
     * @throws Exception
     */
    public function addChild(\_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\NodeInterface $node)
    {
        throw new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\Exception('A prototyped array node can not have concrete children.');
    }
    /**
    ...
    {
        if (\false === $value) {
            throw new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\UnsetKeyException(\sprintf('Unsetting key for path "%s", value: %s.', $this->getPath(), \json_encode($value)));
        }
        foreach ($value as $k => $v) {
            $prototype = $this->getPrototypeForChild($k);
            try {
                $value[$k] = $prototype->finalize($v);
            } catch (\_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\UnsetKeyException $e) {
                unset($value[$k]);
            }
        }
        if (\count($value) < $this->minNumberOfElements) {
            $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The path "%s" should have at least %d element(s) defined.', $this->getPath(), $this->minNumberOfElements));
        ...
        foreach ($value as $k => $v) {
            if (null !== $this->keyAttribute && \is_array($v)) {
                if (!isset($v[$this->keyAttribute]) && \is_int($k) && !$isAssoc) {
                    $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The attribute "%s" must be set for path "%s".', $this->keyAttribute, $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                    ...
                    if (\array_keys($v) === ['value']) {
                        $v = $v['value'];
                        if ($this->prototype instanceof \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\ArrayNode && ($children = $this->prototype->getChildren()) && \array_key_exists('value', $children)) {
                ...
                if (\array_key_exists($k, $normalized)) {
                    $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\DuplicateKeyException(\sprintf('Duplicate key "%s" for path "%s".', $k, $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
            }
        ...
        foreach ($rightSide as $k => $v) {
            ...
            if (!\array_key_exists($k, $leftSide)) {
                if (!$this->allowNewKeys) {
                    $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('You are not allowed to define new elements for path "%s". Please define all elements for this path in one config file.', $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
                $leftSide[$k] = $v;

 ...
 */
class ArrayNode extends \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\BaseNode implements \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
    ...
    protected function finalizeValue($value)
    {
        ...
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    $ex = new \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The child node "%s" at path "%s" must be configured.', $name, $this->getPath()));
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected array, but got %s', $this->getPath(), \gettype($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml($value)
    {
        foreach ($this->xmlRemappings as list($singular, $plural)) {
            ...
            $value[$plural] = \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);
        ...
        foreach ($rightSide as $k => $v) {
            ...
            if (!\array_key_exists($k, $leftSide)) {
                if (!$this->allowNewKeys) {
                    $ex = new \_PhpScoper57793da194f3\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('You are not allowed to define new elements for path "%s". Please define all elements for this path in one config file. If you are trying to overwrite an element, make sure you redefine it with the same name.', $this->getPath()));
                    $ex->setPath($this->getPath());
                    throw $ex;
                }
                $leftSide[$k] = $v;

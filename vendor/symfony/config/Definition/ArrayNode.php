 ...
 */
class ArrayNode extends \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\BaseNode implements \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\PrototypeNodeInterface
{
    ...
    protected function finalizeValue($value)
    {
        ...
        foreach ($this->children as $name => $child) {
            if (!\array_key_exists($name, $value)) {
                if ($child->isRequired()) {
                    $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException(\sprintf('The child node "%s" at path "%s" must be configured.', $name, $this->getPath()));
    ...
    protected function validateType($value)
    {
        if (!\is_array($value) && (!$this->allowFalse || \false !== $value)) {
            $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidTypeException(\sprintf('Invalid type for path "%s". Expected "array", but got "%s"', $this->getPath(), \get_debug_type($value)));
    ...
    protected function normalizeValue($value)
    {
        ...
        if (\count($value) && !$this->ignoreExtraKeys) {
            ...
            $ex = new \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException($msg);
            $ex->setPath($this->getPath());
            throw $ex;
        }
        return $normalized;
    ...
    protected function remapXml(array $value)
    {
        foreach ($this->xmlRemappings as list($singular, $plural)) {
            ...
            $value[$plural] = \_PhpScoperb154859e1be7\Symfony\Component\Config\Definition\Processor::normalizeConfig($value, $singular, $plural);

...
use _PhpScopere7b233920bf2\Psr\Container\ContainerInterface;
use _PhpScopere7b233920bf2\Symfony\Component\EventDispatcher\EventSubscriberInterface;
use _PhpScopere7b233920bf2\Symfony\Component\HttpFoundation\Session\Session;
...
use _PhpScopere7b233920bf2\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException;
 ...
 * @internal
 */
abstract class AbstractSessionListener implements \_PhpScopere7b233920bf2\Symfony\Component\EventDispatcher\EventSubscriberInterface
{
    const NO_AUTO_CACHE_CONTROL_HEADER = 'Symfony-Session-NoAutoCacheControl';
    protected $container;
    private $sessionUsageStack = [];
    private $debug;
    public function __construct(\_PhpScopere7b233920bf2\Psr\Container\ContainerInterface $container = null, bool $debug = \false)
    {
    ...
    public function onKernelRequest(\_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\Event\RequestEvent $event)
    {
        ...
        $this->sessionUsageStack[] = $session instanceof \_PhpScopere7b233920bf2\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() : 0;
    }
    public function onKernelResponse(\_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\Event\ResponseEvent $event)
    {
        ...
        if ($session instanceof \_PhpScopere7b233920bf2\Symfony\Component\HttpFoundation\Session\Session ? $session->getUsageIndex() === \end($this->sessionUsageStack) : !$session->isStarted()) {
            return;
        }
        if ($autoCacheControl) {
        ...
        if ($this->debug) {
            throw new \_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\Exception\UnexpectedSessionUsageException('Session was used while the request was declared stateless.');
        }
        if ($this->container->has('logger')) {
            $this->container->get('logger')->warning('Session was used while the request was declared stateless.');
        }
    }
    public function onFinishRequest(\_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\Event\FinishRequestEvent $event)
    {
        if ($event->isMasterRequest()) {
    ...
    {
        return [
            \_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\KernelEvents::REQUEST => ['onKernelRequest', 128],
            // low priority to come after regular response listeners, but higher than StreamedResponseListener
            \_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\KernelEvents::RESPONSE => ['onKernelResponse', -1000],
            \_PhpScopere7b233920bf2\Symfony\Component\HttpKernel\KernelEvents::FINISH_REQUEST => ['onFinishRequest'],
        ];
    }
    /**

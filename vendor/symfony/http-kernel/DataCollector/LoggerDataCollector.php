 ...
 */
class LoggerDataCollector extends \_PhpScoperf2e2fcfe7ee6\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoperf2e2fcfe7ee6\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
    private $containerPathPrefix;
    private $currentRequest;
    private $requestStack;
    public function __construct($logger = null, string $containerPathPrefix = null, \_PhpScoperf2e2fcfe7ee6\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \_PhpScoperf2e2fcfe7ee6\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \_PhpScoperf2e2fcfe7ee6\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \_PhpScoperf2e2fcfe7ee6\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \_PhpScoperf2e2fcfe7ee6\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {

 ...
 * @final
 */
class LoggerDataCollector extends \_PhpScoper3e1a86bff77f\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoper3e1a86bff77f\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
    private $containerPathPrefix;
    private $currentRequest;
    private $requestStack;
    public function __construct($logger = null, string $containerPathPrefix = null, \_PhpScoper3e1a86bff77f\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \_PhpScoper3e1a86bff77f\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \_PhpScoper3e1a86bff77f\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \_PhpScoper3e1a86bff77f\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \_PhpScoper3e1a86bff77f\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {

 ...
 */
class LoggerDataCollector extends \_PhpScoper1e8bd38a2146\Symfony\Component\HttpKernel\DataCollector\DataCollector implements \_PhpScoper1e8bd38a2146\Symfony\Component\HttpKernel\DataCollector\LateDataCollectorInterface
{
    private $logger;
    private $containerPathPrefix;
    private $currentRequest;
    private $requestStack;
    public function __construct($logger = null, string $containerPathPrefix = null, \_PhpScoper1e8bd38a2146\Symfony\Component\HttpFoundation\RequestStack $requestStack = null)
    {
        if (null !== $logger && $logger instanceof \_PhpScoper1e8bd38a2146\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger = $logger;
        }
    ...
    {
        if ($this->logger instanceof \_PhpScoper1e8bd38a2146\Symfony\Component\HttpKernel\Log\DebugLoggerInterface) {
            $this->logger->clear();
        }
        $this->data = [];
    }
    ...
    private function getContainerDeprecationLogs() : array
    {
        ...
        foreach (\unserialize($logContent) as $log) {
            $log['context'] = ['exception' => new \_PhpScoper1e8bd38a2146\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'], $log['count'])];
    ...
    private function sanitizeLogs(array $logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            if ($exception instanceof \_PhpScoper1e8bd38a2146\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {

 ...
 */
class RegisterLocaleAwareServicesPass implements \_PhpScoperd79f30a2f8be\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\_PhpScoperd79f30a2f8be\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \_PhpScoperd79f30a2f8be\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \_PhpScoperd79f30a2f8be\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}

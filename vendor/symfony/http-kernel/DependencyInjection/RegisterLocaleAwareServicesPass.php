 ...
 */
class RegisterLocaleAwareServicesPass implements \_PhpScoperedfa898a454f\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\_PhpScoperedfa898a454f\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->localeAwareTag) as $id => $tags) {
            $services[] = new \_PhpScoperedfa898a454f\Symfony\Component\DependencyInjection\Reference($id);
        }
        if (!$services) {
            $container->removeDefinition($this->listenerServiceId);
            return;
        }
        $container->getDefinition($this->listenerServiceId)->setArgument(0, new \_PhpScoperedfa898a454f\Symfony\Component\DependencyInjection\Argument\IteratorArgument($services));
    }
}

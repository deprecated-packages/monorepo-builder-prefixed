 ...
 */
class CheckExceptionOnInvalidReferenceBehaviorPass extends \_PhpScopera5e5d165d78c\Symfony\Component\DependencyInjection\Compiler\AbstractRecursivePass
{
    ...
    protected function processValue($value, bool $isRoot = \false)
    {
        ...
        if (\_PhpScopera5e5d165d78c\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $value->getInvalidBehavior() || $this->container->has($id = (string) $value)) {
            return $value;
        }
        ...
        if (isset($this->serviceLocatorContextIds[$currentId])) {
            ...
            foreach ($locator->getArgument(0) as $k => $v) {
                if ($v->getValues()[0] === $value) {
                    ...
                    throw new \_PhpScopera5e5d165d78c\Symfony\Component\DependencyInjection\Exception\ServiceNotFoundException($id, $currentId);
                }
            }
        }
        if ('.' === $currentId[0] && $graph->hasNode($currentId)) {
            foreach ($graph->getNode($currentId)->getInEdges() as $edge) {
                if (!$edge->getValue() instanceof \_PhpScopera5e5d165d78c\Symfony\Component\DependencyInjection\Reference || \_PhpScopera5e5d165d78c\Symfony\Component\DependencyInjection\ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE < $edge->getValue()->getInvalidBehavior()) {
                    continue;
                }

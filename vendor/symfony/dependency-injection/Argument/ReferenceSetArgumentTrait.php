...
trait ReferenceSetArgumentTrait
{
    ...
    public function setValues(array $values)
    {
        foreach ($values as $k => $v) {
            if (null !== $v && !$v instanceof \_PhpScoperd4e30d8318e8\Symfony\Component\DependencyInjection\Reference) {
                throw new \_PhpScoperd4e30d8318e8\Symfony\Component\DependencyInjection\Exception\InvalidArgumentException(\sprintf('A %s must hold only Reference instances, "%s" given.', __CLASS__, \is_object($v) ? \get_class($v) : \gettype($v)));
            }
        }
        $this->values = $values;
    }

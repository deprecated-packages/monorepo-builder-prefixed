 ...
 * @experimental in 4.3
 */
class DataPart extends \_PhpScopere1b4619561f2\Symfony\Component\Mime\Part\TextPart
{
    ...
    public static function fromPath(string $path, string $name = null, string $contentType = null) : self
    {
        ...
        if (null === $contentType) {
            ...
            if (null === self::$mimeTypes) {
                self::$mimeTypes = new \_PhpScopere1b4619561f2\Symfony\Component\Mime\MimeTypes();
            }
            $contentType = self::$mimeTypes->getMimeTypes($ext)[0] ?? 'application/octet-stream';
        }
        if (\false === ($handle = @\fopen($path, 'r', \false))) {
            throw new \_PhpScopere1b4619561f2\Symfony\Component\Mime\Exception\InvalidArgumentException(\sprintf('Unable to open path "%s"', $path));
        }
    ...
    public function __sleep()
    {
        ...
        foreach (['body', 'charset', 'subtype', 'disposition', 'name', 'encoding'] as $name) {
            $r = new \ReflectionProperty(\_PhpScopere1b4619561f2\Symfony\Component\Mime\Part\TextPart::class, $name);
            $r->setAccessible(\true);

...
use _PhpScoperf6f8e31183c3\Symfony\Component\Mime\MessageConverter;
class MessageConverterTest extends \_PhpScoperf6f8e31183c3\PHPUnit\Framework\TestCase
{
    public function testToEmail()
    {
        $file = \file_get_contents(__DIR__ . '/Fixtures/mimetypes/test.gif');
        $email = (new \_PhpScoperf6f8e31183c3\Symfony\Component\Mime\Email())->from('fabien@symfony.com');
        $this->assertSame($email, \_PhpScoperf6f8e31183c3\Symfony\Component\Mime\MessageConverter::toEmail($email));
    ...
    private function assertConversion(\_PhpScoperf6f8e31183c3\Symfony\Component\Mime\Email $expected)
    {
        $r = new \ReflectionMethod($expected, 'generateBody');
        $r->setAccessible(\true);
        $message = new \_PhpScoperf6f8e31183c3\Symfony\Component\Mime\Message($expected->getHeaders(), $r->invoke($expected));
        $converted = \_PhpScoperf6f8e31183c3\Symfony\Component\Mime\MessageConverter::toEmail($message);

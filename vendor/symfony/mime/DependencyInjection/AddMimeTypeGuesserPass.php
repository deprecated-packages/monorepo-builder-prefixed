 ...
 */
namespace _PhpScoperc86a79e2d6b2\Symfony\Component\Mime\DependencyInjection;
 ...
 * @experimental in 4.3
 */
class AddMimeTypeGuesserPass implements \_PhpScoperc86a79e2d6b2\Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface
{
    ...
    public function process(\_PhpScoperc86a79e2d6b2\Symfony\Component\DependencyInjection\ContainerBuilder $container)
    {
        if ($container->has($this->mimeTypesService)) {
            ...
            foreach ($container->findTaggedServiceIds($this->mimeTypeGuesserTag, \true) as $id => $attributes) {
                $definition->addMethodCall('registerGuesser', [new \_PhpScoperc86a79e2d6b2\Symfony\Component\DependencyInjection\Reference($id)]);
            }
        }
    }
}

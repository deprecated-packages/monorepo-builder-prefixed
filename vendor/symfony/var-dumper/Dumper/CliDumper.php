 ...
 */
namespace _PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Dumper;

use _PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor;
 ...
 */
class CliDumper extends \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Dumper\AbstractDumper
{
     ...
     * {@inheritdoc}
     */
    public function dumpScalar(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $value)
    {
        $this->dumpKey($cursor);
        $style = 'const';
        $attr = $cursor->attr;
     ...
     * {@inheritdoc}
     */
    public function dumpString(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $str, $bin, $cut)
    {
        $this->dumpKey($cursor);
        $attr = $cursor->attr;
        if ($bin) {
     ...
     * {@inheritdoc}
     */
    public function enterHash(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $class, $hasChild)
    {
        $this->dumpKey($cursor);
        $attr = $cursor->attr;
        ...
        if (\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type) {
            $prefix = $class && 'stdClass' !== $class ? $this->style('note', $class, $attr) . (empty($attr['cut_hash']) ? ' {' : '') : '{';
        } elseif (\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
        ...
        if (($cursor->softRefCount || 0 < $cursor->softRefHandle) && empty($attr['cut_hash'])) {
            $prefix .= $this->style('ref', (\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type ? '@' : '#') . (0 < $cursor->softRefHandle ? $cursor->softRefHandle : $cursor->softRefTo), ['count' => $cursor->softRefCount]);
        } elseif ($cursor->hardRefTo && !$cursor->refIndex && $class) {
            $prefix .= $this->style('ref', '&' . $cursor->hardRefTo, ['count' => $cursor->hardRefCount]);
        } elseif (!$hasChild && \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE === $type) {
     ...
     * {@inheritdoc}
     */
    public function leaveHash(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $type, $class, $hasChild, $cut)
    {
        if (empty($cursor->attr['cut_hash'])) {
            $this->dumpEllipsis($cursor, $hasChild, $cut);
            $this->line .= \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT === $type ? '}' : (\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE !== $type ? ']' : ($hasChild ? '}' : ''));
        }
        $this->endValue($cursor);
    }
    /**
     ...
     */
    protected function dumpEllipsis(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor, $hasChild, $cut)
    {
        if ($cut) {
            $this->line .= ' …';
            if (0 < $cut) {
     ...
     */
    protected function dumpKey(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (null !== ($key = $cursor->hashKey)) {
            ...
            switch ($cursor->hashType) {
                default:
                case \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_INDEXED:
                    if (self::DUMP_LIGHT_ARRAY & $this->flags) {
                        break;
                    }
                    $style = 'index';
                // no break
                case \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_ASSOC:
                    if (\is_int($key)) {
                    ...
                    break;
                case \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_RESOURCE:
                    $key = "\0~\0" . $key;
                // no break
                case \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor::HASH_OBJECT:
    ...
    protected function endValue(\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Cursor $cursor)
    {
        if (-1 === $cursor->hashType) {
            return;
        }
        if (\_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_INDEXED === $cursor->hashType || \_PhpScoperdd624bbcb4e5\Symfony\Component\VarDumper\Cloner\Stub::ARRAY_ASSOC === $cursor->hashType) {

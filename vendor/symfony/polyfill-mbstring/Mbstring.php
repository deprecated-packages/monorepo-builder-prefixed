 ...
 */
namespace _PhpScoper3e1a86bff77f\Symfony\Polyfill\Mbstring;
...
final class Mbstring
{
    ...
    public static function mb_convert_variables($toEncoding, $fromEncoding, &...$vars)
    {
        ...
        \array_walk_recursive($vars, function (&$v) use(&$ok, $toEncoding, $fromEncoding) {
            if (\false === ($v = \_PhpScoper3e1a86bff77f\Symfony\Polyfill\Mbstring\Mbstring::mb_convert_encoding($v, $toEncoding, $fromEncoding))) {
                $ok = \false;
            }
        });
        ...
        $s = \preg_replace_callback('/&#(?:0*([0-9]+)|x0*([0-9a-fA-F]+))(?!&);?/', function (array $m) use($cnt, $convmap) {
            ...
            for ($i = 0; $i < $cnt; $i += 4) {
                if ($c >= $convmap[$i] && $c <= $convmap[$i + 1]) {
                    return \_PhpScoper3e1a86bff77f\Symfony\Polyfill\Mbstring\Mbstring::mb_chr($c - $convmap[$i + 2]);
                }
            }
            return $m[0];
        }, $s);

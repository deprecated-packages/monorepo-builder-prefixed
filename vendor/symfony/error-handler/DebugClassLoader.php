...
use _PhpScoper9b7e9beaf949\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use _PhpScoper9b7e9beaf949\Doctrine\Persistence\Proxy;
use _PhpScoper9b7e9beaf949\Mockery\MockInterface;
use _PhpScoper9b7e9beaf949\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use _PhpScoper9b7e9beaf949\PHPUnit\Framework\MockObject\MockObject;
use _PhpScoper9b7e9beaf949\Prophecy\Prophecy\ProphecySubjectInterface;
use _PhpScoper9b7e9beaf949\ProxyManager\Proxy\ProxyInterface;
/**
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists('_PhpScoper9b7e9beaf949\\Symfony\\Component\\ErrorHandler\\ErrorHandler');
        \class_exists('_PhpScoper9b7e9beaf949\\Psr\\Log\\LogLevel');
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScoper9b7e9beaf949\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \_PhpScoper9b7e9beaf949\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }

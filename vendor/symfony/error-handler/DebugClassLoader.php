...
use _PhpScopere4e8aecb81b9\Doctrine\Common\Persistence\Proxy as LegacyProxy;
use _PhpScopere4e8aecb81b9\Doctrine\Persistence\Proxy;
use _PhpScopere4e8aecb81b9\Mockery\MockInterface;
use _PhpScopere4e8aecb81b9\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation;
use _PhpScopere4e8aecb81b9\PHPUnit\Framework\MockObject\MockObject;
use _PhpScopere4e8aecb81b9\Prophecy\Prophecy\ProphecySubjectInterface;
use _PhpScopere4e8aecb81b9\ProxyManager\Proxy\ProxyInterface;
/**
...
class DebugClassLoader
{
    ...
    public static function enable() : void
    {
        ...
        \class_exists('_PhpScopere4e8aecb81b9\\Symfony\\Component\\ErrorHandler\\ErrorHandler');
        \class_exists('_PhpScopere4e8aecb81b9\\Psr\\Log\\LogLevel');
    ...
    public static function checkClasses() : bool
    {
        ...
        foreach ($offsets as $getSymbols => $i) {
            ...
            for (; $i < \count($symbols); ++$i) {
                if (!\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\PHPUnit\Framework\MockObject\MockObject::class) && !\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\Prophecy\Prophecy\ProphecySubjectInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\Doctrine\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\ProxyManager\Proxy\ProxyInterface::class) && !\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\Doctrine\Common\Persistence\Proxy::class) && !\is_subclass_of($symbols[$i], \_PhpScopere4e8aecb81b9\Mockery\MockInterface::class)) {
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class) : array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($finalOrInternal || $method->isConstructor() || \false === \strpos($doc, '@param') || \_PhpScopere4e8aecb81b9\PHPUnit\Framework\MockObject\Matcher\StatelessInvocation::class === $class) {
                continue;
            }
